...
songplay_table_create = ("CREATE TABLE IF NOT EXISTS songplays (songplay_id serial PRIMARY KEY, start_time timestamp without time zone REFERENCES time(start_time), user_id varchar REFERENCES users(user_id), level varchar, song_id varchar REFERENCES songs(song_id), artist_id varchar REFERENCES artists(artist_id), session_id varchar, location varchar, user_agent varchar)")
...
time_table_create = ("CREATE TABLE IF NOT EXISTS time (start_time timestamp without time zone PRIMARY KEY, hour int, day int, week int, month int, year int, weekday int)")
